...
ZOMBIE = {
    ...
    "greed": [
        "Will I really get this benefit?",
        "Others already got this, right?",
        "If I delay, will I lose it?",
...
def zombie_reply(text):
    t = text.lower()
    if any(x in t for x in ["blocked", "verify", "urgent", "otp"]):
        return random.choice(ZOMBIE["fear"])
    if any(x in t for x in ["reward", "offer", "bonus"]):
...
def extract_intel(text: str) -> Dict:
    ...
    keywords = ["urgent", "verify", "blocked", "otp", "suspended"]
...
def generate_reply(history, user_text, victim_name):
    ...
    system_prompt = (
        ...
        "You trust the message and fear account loss. "
        "Ask questions and delay actions. "
