...
# # =========================================================
# # 2. AI CLIENTS (STRATEGY: GROQ -> HF -> ZOMBIE)
# # =========================================================
...
# # --- B. HUGGING FACE SETUP (Secondary Backup) ---
# # Uses Phi-3 Mini via API (Zero RAM usage on Render)
...
#     for m in messages[-3:]: # Only take last 3 for speed
...
#     payload = {
#         "inputs": prompt,
#         "parameters": {"max_new_tokens": 40, "return_full_text": False, "temperature": 0.4}
#     }
    
#     try:
#         response = requests.post(HF_API_URL, headers=headers, json=payload, timeout=4)
...
# # =========================================================
# # 3. ZOMBIE MODE (SUPER EXPANDED ARSENAL)
# # =========================================================
# ZOMBIE_RESPONSES = {
#     "upi": [
#         "I typed the UPI but it says 'Invalid Merchant'. Check again?",
...
# # =========================================================
# # 6. RESPONSE GENERATION (Groq -> HF -> Zombie)
# # =========================================================
# def generate_reply(history: List[Any], user_text: str) -> str:
...
#     # --- LEVEL 2: HUGGING FACE (Phi-3 Backup) ---
#     phi_reply = query_huggingface_phi3(messages, user_text)
#     if phi_reply:
#         return phi_reply

#     # --- LEVEL 3: ZOMBIE MODE (Unkillable) ---
#     return zombie_reply(user_text)
...
# # =========================================================
# # 8. API ENDPOINT (CRASH-PROOF & PERMISSIVE)
# # =========================================================
# @app.post("/honey-pot-entry")
# async def entry_point(request: Request, background_tasks: BackgroundTasks):
#     try:
#         body = await request.json()
#     except Exception:
#         # 422 Fix: If JSON is broken, return success anyway
...
#     intel = extract_intel(user_text, session_intelligence[sid])
#     reply = generate_reply(body.get("conversationHistory", []), user_text)

#     background_tasks.add_task(report_intel, sid, intel, 1)
...
# --- B. HUGGING FACE SETUP (Phi-3 with Retry Logic) ---
HF_TOKEN = os.getenv("HUGGINGFACE_API_KEY", "") 
# We use a slightly more reliable URL for serverless inference
HF_API_URL = "https://api-inference.huggingface.co/models/microsoft/Phi-3-mini-4k-instruct"

def query_huggingface_phi3(messages, user_input):
    if not HF_TOKEN or "hf_" not in HF_TOKEN: 
        logger.error("❌ HUGGINGFACE_API_KEY is missing or invalid in Render Settings!")
        return None
    ...
    for attempt in range(3): # Try 3 times
        try:
            ...
            # If Model is Loading (503), Wait and Retry
                ...
                logger.error(f"⚠️ HF API Error {response.status_code}: {response.text}")
                break
...
def generate_reply(history: List[Any], user_text: str) -> str:
    ...
    if phi_reply:
        ...
        logger.warning("⚠️ HF failed, falling back to Zombie")
...
async def entry_point(request: Request, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(report_intel, sid, intel, len(history) + 1 if 'history' in locals() else 1)
